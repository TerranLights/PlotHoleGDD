...
Stats are chosen at chassis assembly (character creation) and can be upgraded later via replacement parts (see Upgradable Robot Body).
...
1. **Receive order** — passenger requests something simple that isn't.
2. **Traverse** — navigate shifting train cars, dodge obstacles/hazards, find the item (or an acceptable substitute).
3. **Complication** — quest glitches, item breaks, wrong car, NPC changes their mind, etc.
4. **Deliver** — return to passenger; non-dialogue exchange resolves the quest regardless of what you "said."
5. **Reward/Failure** — tips, upgrade parts, or chassis wear if you botched it; failure rarely blocks progress, it just makes things weirder.

## Upgradable Robot Body

- Better legs — faster traversal, mentioned under Agility
- Extra arms — carry more trays/items at once (raises effective Core capacity)
- Sarcasm module — unlocks snarkier non-dialogue flavor lines, ties into Empathy/Nerve checks
